...
public class GetProducts extends AsyncTask<Void, Void, Void> {
    ...
    public GetProducts(Context context, onProductsLoadedListener onProductsLoadedListener) {
    ...
    protected Void doInBackground(Void... params) {
        try {
            URL url = new URL(Server.GET_PRODUCTS + "?" + System.currentTimeMillis());
    ...
    protected void onPostExecute(Void aVoid) {
       ...
       // progressDialog.dismiss();
        if (!response.equals("errorOccurred")) {
            onProductsLoadedListener.onProductsLoaded(response);
        } else {
            onProductsLoadedListener.onProductsLoaded(response);
