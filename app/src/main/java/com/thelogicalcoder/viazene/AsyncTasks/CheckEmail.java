 ...
 */
public class CheckEmail extends AsyncTask<Void, Void, Void> {

    private ProgressDialog progressDialog;
    private String response = "", line;
    private onCheckEmailListener onCheckEmailListener;
    private static Context context;
    private HashMap<String, String> registerBody;

    public CheckEmail(Context context, String email, onCheckEmailListener onCheckEmailListener) {
        CheckEmail.context = context;
        progressDialog = new ProgressDialog(context);
        this.onCheckEmailListener = onCheckEmailListener;
    ...
    protected Void doInBackground(Void... params) {
        try {
            URL url = new URL(Server.CHECK_EMAIL + "?" + System.currentTimeMillis());
            ...
            if (httpURLConnection.getResponseCode() == 200) {
                ...
                System.out.println("checkEmail: " + response);
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        if (!response.equals("errorOccurred")) {
            onCheckEmailListener.onEmailChecked(response);
        } else {
            onCheckEmailListener.onEmailChecked(response);
